...
HORIZONS = [5, 15, 30]  # Tier3 may contain multiple; production training uses 30 (see DEFAULT_TRAINING_HORIZON_MINUTES)
...
def optimize_threshold(model, val_seq, val_labels, device, mode='classifier', val_chain=None, norm_mean=None, norm_std=None,
    ...
    """Sweep thresholds on validation set and fit Platt scaling.

    For classifier mode:
      - Fits Platt scaler on val logits.
      - Auto-flips signal if val AUC < 0.5 (anti-predictive model).
      - Selects threshold via chosen objective.
    """
    model.eval()
    ...
    if mode == 'classifier':
        ...
        try:
            ...
            val_auc = 0.5
        # If classifier is directionally inverted on val, flip it.
        if val_auc < 0.5:
            invert_signal = True
            probs = 1.0 - probs
...
def evaluate_model(model, test_seq, test_labels, test_returns, device,
                   ...
                   norm_mean=None, norm_std=None, invert_signal=False):
...
def main():
    ...
    parser.add_argument(
        '--horizon', type=int, default=DEFAULT_TRAINING_HORIZON_MINUTES,
        help='Label horizon in minutes (production default: 30 → Tier3 .../horizon_30min/)',
    )
    ...
    parser.add_argument('--threshold-objective', choices=['f1', 'balanced_acc', 'mcc'], default='balanced_acc',
                        help='Objective used to select validation threshold')
