...
from .ohlc_features import OhlcFeatureExtractor
...
class MasterFeatureExtractor:
    def __init__(
        ...
        self.preprocess_counters = {
            "excluded_greek_cols_dropped": 0,
            "excluded_trade_cols_dropped": 0,
            "quote_ffill_rows": 0,
        }
    
    def preprocess_greek_df(self, df: pd.DataFrame) -> pd.DataFrame:
        """Remove excluded Greek columns (speed, vera)."""
        to_drop = [c for c in self.excluded_greek_columns if c in df.columns]
        self.preprocess_counters["excluded_greek_cols_dropped"] += len(to_drop)
        return df.drop(columns=to_drop, errors='ignore')
    
    def preprocess_trade_df(self, df: pd.DataFrame) -> pd.DataFrame:
        """Drop dead TQ cols and forward-fill quote fields for sparse minutes."""
        to_drop = [c for c in self.excluded_trade_columns if c in df.columns]
        self.preprocess_counters["excluded_trade_cols_dropped"] += len(to_drop)
        out = df.drop(columns=to_drop, errors='ignore').copy()
        if out.empty:
            return out

        # Apply bounded forward-fill on quote fields to reduce zero-vectors when
        # minute snapshots have intermittent quote updates.
        ts_col = next((c for c in ("trade_timestamp", "quote_timestamp", "timestamp") if c in out.columns), None)
        if ts_col is not None:
            out[ts_col] = pd.to_datetime(out[ts_col], errors="coerce")
            out = out.sort_values(ts_col)
        quote_cols = [c for c in ("bid", "ask", "bid_size", "ask_size", "bid_exchange", "ask_exchange") if c in out.columns]
        if quote_cols:
            before = out[quote_cols].isna().sum().sum()
            out[quote_cols] = out[quote_cols].ffill(limit=5)
            after = out[quote_cols].isna().sum().sum()
            self.preprocess_counters["quote_ffill_rows"] += int(max(0, before - after))
        return out
    ...
    def extract_flat_features(
        ...
        Args:
            ...
            ohlc_df: 1-minute OHLC snapshot for chain-level OHLC dynamics
            open_interest: Current open interest (for volume anomaly detection)
        
        Returns:
            Feature array of shape (295 + optional phase1 features,)
        """
        n_features = 350 if self.include_phase1 else 295
        ...
        # OHLC dynamics block (25 features; zeros when OHLC unavailable)
        ohlc_features = self.ohlc_extractor.extract(ohlc_df if ohlc_df is not None else pd.DataFrame())
        features[idx:idx + 25] = ohlc_features
        idx += 25
        ...
        if self.include_phase1 and trade_df is not None:
            ...
            assert idx == 350, f"Expected 350 total features, got {idx}"
        else:
            assert idx == 295, f"Expected 295 total features, got {idx}"
    ...
    def extract(
        ...
        Args:
            ...
            ohlc_df: 1-minute OHLC snapshot aligned to this timestamp
        ...
        """
        flat_features = self.extract_flat_features(greek_df, trade_df, ohlc_df, open_interest)
    ...
    def extract_batch(
        ...
        ohlc_dfs: Optional[List[pd.DataFrame]] = None,
        ...
        Args:
            ...
            ohlc_dfs: List of OHLC snapshots (same length as greek_dfs)
...
def extract_all_features(
    ...
    Args:
        ...
        ohlc_df: OHLC data snapshot for OHLC dynamics block
    ...
    return extractor.extract(greek_df, trade_df, ohlc_df=ohlc_df)
