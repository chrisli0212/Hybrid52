"""
Hybrid51 Feature Configuration
Defines all 270 features with their groups, indices, and descriptions.
"""
...
CHAIN_2D_CONFIG = {
    "n_greeks": 5,
    "n_strikes": 30,
    "n_timesteps": 30,  # output T for downstream (G,S,T); build_sequences may use 15-min lookback and pad to 30
...
def get_group_indices(group: FeatureGroup) -> range:
    return FEATURE_GROUPS[group].indices


def get_subgroup_indices(group: FeatureGroup, subgroup: str) -> Tuple[int, int]:
    return FEATURE_GROUPS[group].subgroups[subgroup]


def print_feature_summary():
    print("=" * 80)
    print("HYBRID51 FEATURE CONFIGURATION")
    print("=" * 80)
    ...
    for group, config in FEATURE_GROUPS.items():
        print(f"\n{config.name} [{config.start_idx}:{config.end_idx}] ({config.num_features} features)")
        print(f"  {config.description}")
        for subgroup, (start, end) in config.subgroups.items():
            print(f"    - {subgroup}: [{start}:{end}] ({end-start} features)")
    
    print("\n" + "-" * 40)
    print("2D CHAIN CONFIG (for Agent-2D):")
    print("-" * 40)
    print(f"  Shape: ({CHAIN_2D_CONFIG['n_greeks']}, {CHAIN_2D_CONFIG['n_strikes']}, {CHAIN_2D_CONFIG['n_timesteps']})")
    print(f"  Greeks: {CHAIN_2D_CONFIG['greeks']}")
    print(f"  Delta range: {CHAIN_2D_CONFIG['delta_range']}")
...
if __name__ == "__main__":
    ...
    names = get_feature_names()
    print(f"\n\nGenerated {len(names)} feature names")
    print("\nFirst 20 features:")
    for i, name in enumerate(names[:20]):
        print(f"  [{i:3d}] {name}")
    print("\nLast 20 features:")
    for i, name in enumerate(names[-20:], start=TOTAL_FEATURES-20):
        print(f"  [{i:3d}] {name}")
