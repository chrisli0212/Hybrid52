...
def _split_calls_puts(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Robust call/put split across right/cp_sign encodings.
    Falls back to delta sign when side columns are unavailable/noisy.
    """
...
def calculate_max_gamma_strikes(df: pd.DataFrame) -> Dict[str, float]:
    ...
    if len(calls) > 0:
        call_gamma_by_strike = calls.groupby('strike')['gamma'].sum().abs()
        if len(call_gamma_by_strike) > 0:
            features["call_max_gamma_strike"] = call_gamma_by_strike.idxmax()
    
    if len(puts) > 0:
        put_gamma_by_strike = puts.groupby('strike')['gamma'].sum().abs()
...
def calculate_wall_distances(df: pd.DataFrame) -> Dict[str, float]:
    ...
    if 'underlying_price' in df.columns:
        underlying = pd.to_numeric(df['underlying_price'], errors='coerce').dropna()
        underlying = float(underlying.iloc[0]) if len(underlying) > 0 else np.nan
    elif 'strike' in df.columns:
        underlying = float(pd.to_numeric(df['strike'], errors='coerce').median())
    else:
        return features
    if not np.isfinite(underlying) or abs(underlying) < 1e-8:
        return features
