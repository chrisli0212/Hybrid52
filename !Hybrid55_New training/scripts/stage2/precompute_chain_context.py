...
Usage:
    python scripts/stage2/precompute_chain_context.py --symbol SPXW --horizon 30
"""
...
@torch.no_grad()
def _infer_batched(model, sequences, chain_2d, device, batch_size=1024, invert_signal=False):
...
def main():
    ...
    logger.info(f"  Model loaded, feat_dim={ckpt.get('feat_dim', 325)}, cls_input_dim={cls_in}, invert={invert_signal}")
    ...
    for split in ['train', 'val', 'test']:
        ...
        ts_path = chain_dir / f'{split}_timestamps.npy'
        if ts_path.exists():
            timestamps = np.load(ts_path).astype(np.int64)
        else:
            logger.warning(f"  {split}: timestamps missing, using synthetic indices")
        ...
        logits, probs = _infer_batched(
            model, seq, chain, device, args.batch_size, invert_signal=invert_signal
        )
        result[f'{split}_logits'] = logits
        result[f'{split}_probs'] = probs
        result[f'{split}_labels'] = labels.astype(np.int64)
        result[f'{split}_timestamps'] = timestamps
