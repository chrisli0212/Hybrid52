#!/usr/bin/env python3
"""
Stage 2: Per-Agent Cross-Symbol Fusion (standard agents A/B/C/K/TQ/H)
...
from hybrid55_models.cross_symbol_agent_fusion import CrossSymbolAgentFusion
from hybrid55_models.tlt_gated_agent_fusion import TLTGatedAgentFusion
from config.feature_subsets import assert_supported_schema
...
STANDARD_AGENTS = ['A', 'B', 'C', 'K', 'TQ', 'H', 'M']
...
class BinaryIndependentAgent(nn.Module):
    def __init__(self, agent_type, feat_dim=311, temporal_dim=128, dropout=0.2,
...
def _build_model_from_ckpt(ckpt, agent_type: str, device: torch.device, symbol: str):
    ...
    if 'feat_dim' in ckpt:
        ...
        # Infer from max index for compatibility with older checkpoints.
        feat_dim = 650 if int(state['base._feat_idx'].max().item()) >= 311 else 311
    else:
        feat_dim = 311 if symbol == 'SPXW' else 650
...
def _load_split(symbol: str, horizon: int, split: str):
    ...
    if meta_path.exists():
        try:
            schema_version = json.loads(meta_path.read_text()).get("feature_schema_version", FEATURE_SCHEMA_VERSION)
            assert_supported_schema(str(schema_version))
        except Exception as exc:
            raise RuntimeError(f"{symbol} h{horizon}: incompatible or unreadable metadata schema: {exc}") from exc
