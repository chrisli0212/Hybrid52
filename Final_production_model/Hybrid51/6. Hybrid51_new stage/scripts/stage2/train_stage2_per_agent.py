...
Usage:
    ...
    python scripts/stage2/train_stage2_per_agent.py --target SPXW --horizon 15 --agents A B K
"""
...
def _load_stage1_model(symbol: str, agent: str, horizon: int, device: torch.device):
    ...
    return model, feat_dim, threshold


@torch.no_grad()
def _infer_logits_probs(model, sequences, norm_mean, norm_std, device, batch_size=2048):
    n = len(sequences)
    logits_all = []
    norm_mean_t = None
    norm_std_t = None
...
def _load_split(symbol: str, horizon: int, split: str):
    ...
    return seq, labels, norm_mean, norm_std
...
def _build_design_matrix(agent: str, horizon: int, split: str, device: torch.device,
    ...
    for sym in all_symbols:
        t_sym = time.time()
        seq, labels, norm_mean, norm_std = _load_split(sym, horizon, split)
        if split_labels is None:
            split_labels = labels

        model = stage1_models[sym]
        lg, pb = _infer_logits_probs(model, seq, norm_mean, norm_std, device, batch_size)
    ...
    for sym in peer_symbols:
        parts.append(logits_per_sym[sym][:n].reshape(-1, 1))
        parts.append(probs_per_sym[sym][:n].reshape(-1, 1))

    for sym in peer_symbols:
        diff = logit_spxw - logits_per_sym[sym][:n]
        parts.append(diff.reshape(-1, 1))

    ctx_logits = chain_context[f'{split}_logits'][:n].reshape(-1, 1)
    ctx_probs = chain_context[f'{split}_probs'][:n].reshape(-1, 1)
    parts.extend([ctx_logits, ctx_probs])

    X = np.concatenate(parts, axis=1).astype(np.float32)
    y = split_labels[:n].astype(np.float32)
    return X, y
...
def _build_tlt_context(agent: str, horizon: int, split: str, device: torch.device,
    ...
    seq, _, norm_mean, norm_std = _load_split('TLT', horizon, split)
    tlt_logits, tlt_probs = _infer_logits_probs(tlt_model, seq, norm_mean, norm_std, device, batch_size)
    return np.stack([tlt_logits, tlt_probs], axis=1).astype(np.float32)  # (n, 2)
...
def _build_design_matrix_tlt(agent: str, horizon: int, split: str, device: torch.device,
    ...
    X, y = _build_design_matrix(agent, horizon, split, device, stage1_models,
                                 chain_context, peer_symbols, batch_size)
    tlt_ctx = _build_tlt_context(agent, horizon, split, device, tlt_model, batch_size)
    n = min(len(X), len(tlt_ctx))
    X = np.concatenate([X[:n], tlt_ctx[:n]], axis=1).astype(np.float32)
    y = y[:n]
    return X, y
...
def train_agent_fusion(agent: str, target: str, horizon: int, device: torch.device,
    ...
    for sym in all_symbols:
        try:
            model, _, _ = _load_stage1_model(sym, agent, horizon, device)
            stage1_models[sym] = model
            logger.info(f"  Loaded Stage1 {sym} Agent {agent}")
    ...
    X_train, y_train = _build_design_matrix(agent, horizon, 'train', device, stage1_models, chain_context, peer_symbols, stage1_batch)
    X_val,   y_val   = _build_design_matrix(agent, horizon, 'val',   device, stage1_models, chain_context, peer_symbols, stage1_batch)
    X_test,  y_test  = _build_design_matrix(agent, horizon, 'test',  device, stage1_models, chain_context, peer_symbols, stage1_batch)
...
def train_agent_fusion_tlt_gated(agent: str, target: str, horizon: int, device: torch.device,
    ...
    try:
        tlt_model, _, _ = _load_stage1_model('TLT', agent, horizon, device)
