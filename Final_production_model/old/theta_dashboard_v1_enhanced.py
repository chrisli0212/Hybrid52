"""
Theta Options Intelligence Dashboard — Decoupled CSV Architecture
    ...
    return html.Div(
        ...
        children=[
            html.Div("MODEL PREDICTION", style={
                ...
                "color": theta_dashboard.C["accent"], "marginBottom": "12px",
            }),
            html.Div("Waiting for prediction data...", style={
                "fontSize": "16px", "color": theta_dashboard.C["text_muted"], "marginBottom": "8px",
            }),
            html.Div("Start prediction_service.py to enable model predictions.", style={
                "fontSize": "13px", "color": theta_dashboard.C["text_muted"], "marginBottom": "12px",
            }),
            html.Code("python prediction_service.py", style={
                ...
                "borderRadius": "4px",
                "fontSize": "13px",
                "color": theta_dashboard.C["accent"],
            }),
        ]
    )
...
def _create_alert_panel():
    ...
    severity_colors = {
        "high": theta_dashboard.C["put"],
        "medium": theta_dashboard.C["warning"],
        "info": theta_dashboard.C["call"],
    }
    ...
    if not alerts:
        rows = [html.Div(
            ...
            style={"color": theta_dashboard.C["text_muted"], "fontSize": "12px", "padding": "8px"}
        )]
    else:
        rows = []
        for a in reversed(alerts):
            sev_color = severity_colors.get(a.get("severity", "info"), theta_dashboard.C["text_muted"])
            rows.append(html.Div(
                style={
                    "display": "flex", "gap": "8px", "alignItems": "flex-start",
                    "padding": "5px 0",
                    "borderBottom": f"1px solid {theta_dashboard.C['border']}",
                },
                children=[
                    html.Span(a["ts"], style={"fontSize": "11px", "color": theta_dashboard.C["text_muted"], "minWidth": "65px"}),
                    html.Span("*", style={"color": sev_color, "fontSize": "10px", "marginTop": "2px"}),
                    html.Span(a["msg"], style={"fontSize": "12px", "color": theta_dashboard.C["text"]}),
                ]
            ))
    ...
    return html.Div(
        ...
        children=[
            html.Div("ALERTS", style={
                "fontSize": "11px", "fontWeight": 700, "letterSpacing": "0.5px",
                "color": theta_dashboard.C["warning"], "marginBottom": "8px",
            }),
            *rows,
        ]
    )
...
def _create_decision_engine_panel(model_out, pred_history_roll):
    ...
    if not model_out:
        return html.Div(
            ...
            children=[
                html.Div("DECISION ENGINE", style={"fontSize": "11px", "fontWeight": 700, "letterSpacing": "0.5px", "color": theta_dashboard.C["accent"]}),
                html.Div("Awaiting model data...", style={"color": theta_dashboard.C["text_muted"], "fontSize": "13px", "marginTop": "10px"}),
            ]
        )
    ...
    if suppressed or not ok:
        ...
        action_color = theta_dashboard.C["text_muted"]
        ...
        action_color = theta_dashboard.C["warning"]
        ...
        action_color = theta_dashboard.C["call"]
        ...
        action_text = ">> SKIP"
        action_color = theta_dashboard.C["put"]
    ...
    def _check_row(label, passed, detail_text):
        ...
        color = theta_dashboard.C["call"] if passed else theta_dashboard.C["put"]
        return html.Div(
            ...
            children=[
                ...
                html.Span(f"{label}:", style={"fontSize": "12px", "color": theta_dashboard.C["text_muted"], "minWidth": "90px"}),
    ...
    return html.Div(
        ...
        children=[
            html.Div("DECISION ENGINE", style={
                "fontSize": "11px", "fontWeight": 700, "letterSpacing": "0.5px",
                "color": theta_dashboard.C["accent"], "marginBottom": "10px",
                "borderBottom": f"1px solid {theta_dashboard.C['border']}", "paddingBottom": "6px",
            }),
            ...
            html.Div(style={
                "borderTop": f"1px solid {theta_dashboard.C['border']}",
                ...
                html.Div(
                    ...
                    children=[
                        html.Span(f">> SUGGESTED SIZE: {size_mult}", style={
                            "fontSize": "13px", "fontWeight": 700, "color": theta_dashboard.C["text"],
                        }),
                    ]
                ),
                html.Div(size_note, style={"fontSize": "11px", "color": theta_dashboard.C["text_muted"], "marginTop": "2px"}),
            ]),
            html.Div(style={
                "borderTop": f"1px solid {theta_dashboard.C['border']}",
                ...
                html.Div(action_detail, style={
                    "fontSize": "12px", "color": theta_dashboard.C["text_muted"], "marginTop": "3px",
                }),
            ]),
        ]
    )
...
def _create_agent_hud_strip(model_out, symbol, agg_df):
    ...
    direction = "UP" if prob >= 0.5 else "DOWN"
    dir_color = theta_dashboard.C["call"] if prob >= 0.5 else theta_dashboard.C["put"]

    if suppressed or not ok:
        dir_badge = html.Span("PAUSED SUPPRESSED", style={"color": theta_dashboard.C["text_muted"], "fontWeight": 700})
    else:
    ...
    # Confidence badge
    if confidence >= 0.7:
        conf_badge_color = theta_dashboard.C["call"]
        conf_label = "HIGH"
    elif confidence >= 0.5:
        conf_badge_color = theta_dashboard.C["warning"]
        conf_label = "MED"
    else:
        conf_badge_color = theta_dashboard.C["put"]
        conf_label = "LOW"

    # Regime
    regime_text = "CALM" if (vix_valid and quality > 0.5) else "STRESSED"
    regime_color = theta_dashboard.C["call"] if regime_text == "CALM" else theta_dashboard.C["warning"]
    ...
    for k in agent_keys:
        ...
        color = theta_dashboard.C["call"] if is_up else theta_dashboard.C["put"]
        if suppressed or not ok:
            color = theta_dashboard.C["text_muted"]
    ...
    # Tracking status
    tracking_text = "--"
    tracking_color = theta_dashboard.C["text_muted"]
        ...
        if len(sym_agg) >= 2 and "spot" in sym_agg.columns:
            ...
            if len(recent_spot) >= 2:
                ...
                if (spot_move >= 0 and prob >= 0.5) or (spot_move < 0 and prob < 0.5):
                    tracking_text = "OK"
                    tracking_color = theta_dashboard.C["call"]
                else:
                    tracking_text = "X"
                    tracking_color = theta_dashboard.C["put"]
    ...
    return html.Div(
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                    html.Span(f"{symbol} {spot_text}", style={
                        "fontSize": "14px", "fontWeight": 700, "color": theta_dashboard.C["text"],
                    }),
                    html.Span("|", style={"color": theta_dashboard.C["border"]}),
                    dir_badge,
                    html.Span("|", style={"color": theta_dashboard.C["border"]}),
                    html.Span("conf: ", style={"color": theta_dashboard.C["text_muted"], "fontSize": "12px"}),
                    ...
                    html.Span("|", style={"color": theta_dashboard.C["border"]}),
                    html.Span("regime: ", style={"color": theta_dashboard.C["text_muted"], "fontSize": "12px"}),
                    html.Span(regime_text, style={"color": regime_color, "fontSize": "12px", "fontWeight": 600}),
                    html.Span("|", style={"color": theta_dashboard.C["border"]}),
                    html.Span(countdown_text, style={"color": theta_dashboard.C["text_muted"], "fontSize": "12px"}),
                ]
            ),
            # Row 2
            html.Div(
                ...
                children=[
                    *agent_votes,
                    ...
                    html.Span(f"consensus: {up_count}/7", style={
                        "color": theta_dashboard.C["text_sec"], "fontSize": "12px",
                    }),
                    html.Span("|", style={"color": theta_dashboard.C["border"]}),
                    html.Span("tracking: ", style={"color": theta_dashboard.C["text_muted"], "fontSize": "12px"}),
            ...
            _build_confidence_decomposition_row(model_out, theta_dashboard.C),
        ]
    )
...
def _create_sizing_guidance(model_out, pred_history_roll):
    ...
    if not model_out:
        return html.Div(
            ...
            children=[
                html.Div("POSITION SIZING", style={"fontSize": "11px", "fontWeight": 700, "letterSpacing": "0.5px", "color": theta_dashboard.C["accent"]}),
                html.Div("Awaiting model data...", style={"color": theta_dashboard.C["text_muted"], "fontSize": "13px", "marginTop": "10px"}),
            ]
        )
    ...
    for i, (tier_label, lv, hv) in enumerate(tiers):
        ...
        bg = f"{theta_dashboard.C['accent']}18" if is_active else "transparent"
        table_rows.append(html.Tr(style={"backgroundColor": bg}, children=[
            html.Td(tier_label, style={"padding": "4px 8px", "fontSize": "12px", "color": theta_dashboard.C["text"], "fontWeight": 700 if is_active else 400}),
            html.Td(lv, style={"padding": "4px 8px", "fontSize": "12px", "color": theta_dashboard.C["call"], "textAlign": "center"}),
            html.Td(hv, style={"padding": "4px 8px", "fontSize": "12px", "color": theta_dashboard.C["warning"], "textAlign": "center"}),
        ]))
        ...
        children=[
            ...
            html.Div(style={"textAlign": "center", "marginBottom": "12px"}, children=[
                html.Div(current_mult, style={
                    ...
                    "color": theta_dashboard.C["call"] if current_mult not in ("0x",) else theta_dashboard.C["text_muted"],
                }),
                html.Div("Recommended Size", style={"fontSize": "11px", "color": theta_dashboard.C["text_muted"]}),
            ]),
            html.Table(
                style={"width": "100%", "borderCollapse": "collapse"},
                children=[
                    html.Thead(html.Tr(children=[
                        html.Th("Confidence", style={"padding": "4px 8px", "fontSize": "11px", "color": theta_dashboard.C["text_muted"], "textAlign": "left", "borderBottom": f"1px solid {theta_dashboard.C['border']}"}),
                        html.Th("Low Vol", style={"padding": "4px 8px", "fontSize": "11px", "color": theta_dashboard.C["text_muted"], "textAlign": "center", "borderBottom": f"1px solid {theta_dashboard.C['border']}"}),
                        html.Th("High Vol", style={"padding": "4px 8px", "fontSize": "11px", "color": theta_dashboard.C["text_muted"], "textAlign": "center", "borderBottom": f"1px solid {theta_dashboard.C['border']}"}),
                    ])),
            ...
            html.Div(style={
                ...
                html.Div("CIRCUIT BREAKER", style={"fontSize": "10px", "fontWeight": 700, "color": theta_dashboard.C["text_muted"], "letterSpacing": "0.5px"}),
                html.Div(f"Signals today: {non_supp_count}", style={"fontSize": "12px", "color": theta_dashboard.C["text_sec"], "marginTop": "3px"}),
                html.Div(f"Regime: {'Low Vol' if low_vol else 'High Vol / Stressed'}", style={
                    "fontSize": "12px",
                    "color": theta_dashboard.C["call"] if low_vol else theta_dashboard.C["warning"],
                    "marginTop": "2px",
                }),
            ]),
        ]
...
def _create_model_health_panel(model_out, pred_history_roll):
    ...
    def _mini_sparkline(values, color, height=35, width=120):
        ...
        if not values or len(values) < 2:
            return html.Div("--", style={"color": theta_dashboard.C["text_muted"], "fontSize": "11px"})
        fig = go.Figure()
    ...
    def _health_card(title, value, subtitle, sparkline_values=None, spark_color=None, value_color=None):
        children = [
            html.Div(title, style={"fontSize": "10px", "fontWeight": 700, "color": theta_dashboard.C["text_muted"], "letterSpacing": "0.5px", "textTransform": "uppercase"}),
            html.Div(value, style={"fontSize": "22px", "fontWeight": 800, "color": value_color or theta_dashboard.C["text"], "marginTop": "3px"}),
            html.Div(subtitle, style={"fontSize": "11px", "color": theta_dashboard.C["text_muted"], "marginTop": "2px"}),
        ]
    ...
    acc_color = theta_dashboard.C["call"] if accuracy >= 0.55 else (theta_dashboard.C["warning"] if accuracy >= 0.45 else theta_dashboard.C["put"])
    qual_color = theta_dashboard.C["call"] if quality >= 0.6 else (theta_dashboard.C["warning"] if quality >= 0.4 else theta_dashboard.C["put"])
    comp_color = theta_dashboard.C["call"] if completeness >= 0.8 else (theta_dashboard.C["warning"] if completeness >= 0.5 else theta_dashboard.C["put"])
    lat_color = theta_dashboard.C["call"] if (latency is not None and latency < 500) else theta_dashboard.C["warning"]
    ...
    cards = [
        ...
        _health_card("Stage1 Missing", str(missing_stage1), "Missing features",
                      value_color=theta_dashboard.C["call"] if missing_stage1 == 0 else theta_dashboard.C["warning"]),
        _health_card("History Depth", str(len(hist)), f"{len(non_supp)} non-suppressed",
                      value_color=theta_dashboard.C["text"]),
    ]
...
def _create_signal_meters(model_out):
    ...
    fig.add_trace(
        go.Indicator(
            ...
            title={"text": f"Stage 3 {s3_arrow} ({source_state})  |  Confidence {confidence*100:.0f}%  |  d0.5: {s3_delta_text}", "font": {"size": 16, "color": theta_dashboard.C["text"]}},
            gauge={
                "shape": "angular",
    ...
    for idx, (label, key) in enumerate(agents):
        ...
        if neutral_mode:
            s2_bar = theta_dashboard.C["text_muted"]
            title_color = theta_dashboard.C["text_muted"]
        else:
            s2_bar = theta_dashboard.C["call"] if is_up else theta_dashboard.C["put"]
            title_color = theta_dashboard.C["call"] if is_up else theta_dashboard.C["put"]
...
def _create_agent_agreement_bar(model_out):
    ...
    up_color = theta_dashboard.C["text_muted"] if neutral_mode else theta_dashboard.C["call"]
    down_color = theta_dashboard.C["text_muted"] if neutral_mode else theta_dashboard.C["put"]
    ...
    return html.Div(
        ...
        children=[
            html.Div("AGENT AGREEMENT", style={
                ...
                "color": theta_dashboard.C["text_muted"], "marginBottom": "6px",
            }),
    ...
    if not suppressed and ok and len(hist) >= 5:
        ...
        if len(recent_spots) >= 2:
            ...
            if predicted_up == moved_up:
                ...
                tracking_color = theta_dashboard.C["warning"]
...
def _model_signal_card(model_out):
    ...
    if suppressed:
        ...
        color = theta_dashboard.C["warning"]
    elif ok:
        direction = "BULL" if pred == 1 else "BEAR"
        color = theta_dashboard.C["call"] if pred == 1 else theta_dashboard.C["put"]
    ...
    return html.Div(
        ...
        children=[
            html.Div(
                style={"display": "flex", "justifyContent": "space-between", "alignItems": "center"},
                children=[
                    html.Span("MODEL", style={"fontSize": "11px", "fontWeight": 700, "letterSpacing": "0.5px", "color": theta_dashboard.C["text_muted"]}),
                    html.Span(label, style={"fontSize": "16px", "fontWeight": 700, "color": color}),
                ]
            ),
            html.Div(
                " | ".join(text_parts),
                style={"fontSize": "11px", "color": theta_dashboard.C["text_muted"], "marginTop": "6px"},
            ),
        ]
    )
...
def _fmt_premium(val):
    """Format premium values as $XXM or $XXB. Returns '--' for NaN/None."""
...
def _fmt_premium_abs(val):
    """Format premium values as $XXM or $XXB (no sign). Returns '--' for NaN/None."""
    if val is None or (isinstance(val, float) and np.isnan(val)):
        return "--"
    av = abs(val)
    if av >= 1e9:
...
def _stat_tile(label, value_text, color, sub_text=None):
    """Create a single stat tile for the header."""
    children = [
        html.Div(label, style={
            'color': theta_dashboard.C['text_muted'], 'fontSize': '10px',
            'textTransform': 'uppercase', 'letterSpacing': '0.5px', 'marginBottom': '2px',
        }),
        html.Div(value_text, style={
            'fontSize': '18px', 'fontWeight': 'bold', 'color': color, 'lineHeight': '1.2',
        }),
    ]
    if sub_text:
        children.append(html.Div(sub_text, style={
            'fontSize': '10px', 'color': theta_dashboard.C['text_muted'], 'marginTop': '1px',
        }))
    return html.Div(style={'textAlign': 'center', 'minWidth': '80px', 'padding': '4px 8px'}, children=children)


def _build_ticker_ribbon(all_stats):
    """Build a horizontal ticker ribbon showing all symbols."""
    if not all_stats:
        return None
    items = []
    for sym in ['SPXW', 'SPY', 'QQQ', 'IWM', 'TLT', 'VIXW']:
        ...
        chg_color = theta_dashboard.C['call'] if chg >= 0 else theta_dashboard.C['put']
        pc = st.get('pc_ratio', 0.0)
        pc_color = theta_dashboard.C['put'] if pc > 1.0 else theta_dashboard.C['call']
        items.append(html.Div(
            style={
                'display': 'flex', 'alignItems': 'center', 'gap': '8px',
                'padding': '6px 14px',
                'borderRight': f'1px solid {theta_dashboard.C["border"]}',
                'flex': '1', 'justifyContent': 'center',
            },
            children=[
                html.Span(sym, style={'fontWeight': 700, 'fontSize': '12px', 'color': theta_dashboard.C['text']}),
                html.Span(f"${st['price']:.2f}" if pd.notna(st.get('price')) else "--", style={'fontSize': '13px', 'color': theta_dashboard.C['text'], 'fontWeight': 600}),
                html.Span(f"{chg:+.2f}%" if pd.notna(chg) else "--", style={'fontSize': '11px', 'color': chg_color, 'fontWeight': 600}),
                html.Span(f"P/C {pc:.2f}" if pc != 0 else "P/C --", style={'fontSize': '10px', 'color': pc_color}),
            ]
        ))
    if not items:
        return None
    return html.Div(
        style={
            'display': 'flex', 'backgroundColor': theta_dashboard.C['bg_card'],
            'border': f'1px solid {theta_dashboard.C["border"]}',
...
def _build_premium_flow_bar(st):
    """Build a premium flow visualization bar."""
    ...
    return html.Div(
        ...
        children=[
            html.Div('PREMIUM FLOW', style={
                ...
                'color': theta_dashboard.C['text_muted'], 'marginBottom': '8px',
            }),
            html.Div(style={'display': 'flex', 'alignItems': 'center', 'gap': '8px'}, children=[
                html.Span(f"CALL {_fmt_premium_abs(call_p)} ({call_pct:.0f}%)", style={
                    'fontSize': '11px', 'fontWeight': 600, 'color': theta_dashboard.C['call'], 'minWidth': '130px',
                }),
                html.Div(style={
                    'flex': '1', 'height': '22px', 'display': 'flex', 'borderRadius': '4px', 'overflow': 'hidden',
                }, children=[
                    html.Div(style={
                        'width': f'{call_pct:.1f}%', 'backgroundColor': theta_dashboard.C['call'],
                        'opacity': '0.7', 'transition': 'width 0.3s',
                    }),
                    html.Div(style={
                        'width': f'{put_pct:.1f}%', 'backgroundColor': theta_dashboard.C['put'],
                        'opacity': '0.7', 'transition': 'width 0.3s',
                    }),
                ]),
                html.Span(f"PUT {_fmt_premium_abs(put_p)} ({put_pct:.0f}%)", style={
                    'fontSize': '11px', 'fontWeight': 600, 'color': theta_dashboard.C['put'],
                    'minWidth': '130px', 'textAlign': 'right',
                }),
            ]),
            html.Div(f"Net: {_fmt_premium(st.get('net_premium', 0.0))} | Total: {_fmt_premium_abs(total)}", style={
                'fontSize': '10px', 'color': theta_dashboard.C['text_muted'], 'marginTop': '4px', 'textAlign': 'center',
            }),
        ]
    )


def _build_regime_badge(net_gex, vix_level=None):
    """Build market regime indicator badge."""
    if net_gex >= 0:
        regime_text = "POSITIVE GAMMA"
        regime_sub = "Mean-Reverting Regime — Dealer hedging dampens moves"
        regime_color = theta_dashboard.C['call']
        border_color = theta_dashboard.C['call']
    else:
        regime_text = "NEGATIVE GAMMA"
        regime_sub = "Trend-Following Regime — Dealer hedging amplifies moves"
        regime_color = theta_dashboard.C['put']
        border_color = theta_dashboard.C['put']
    children = [
        html.Span(regime_text, style={'fontWeight': 700, 'fontSize': '13px', 'color': regime_color}),
        html.Span(f" — {regime_sub}", style={'fontSize': '12px', 'color': theta_dashboard.C['text_sec']}),
    ]
    if vix_level and vix_level > 0:
        vix_color = theta_dashboard.C['put'] if vix_level > 25 else (theta_dashboard.C['warning'] if vix_level > 18 else theta_dashboard.C['call'])
        children.append(html.Span(f"  |  VIX: {vix_level:.1f}", style={'fontSize': '12px', 'color': vix_color, 'fontWeight': 600}))
    return html.Div(
        style={
            'backgroundColor': theta_dashboard.C['bg_card'],
...
def _build_cross_symbol_table(all_stats):
    """Build a cross-symbol summary table for ALL view."""
    if not all_stats:
        return None
    header_style = {
        'fontSize': '10px', 'fontWeight': 700, 'letterSpacing': '0.5px',
        'color': theta_dashboard.C['text_muted'], 'textTransform': 'uppercase',
        'padding': '8px 10px', 'textAlign': 'right', 'borderBottom': f'1px solid {theta_dashboard.C["border"]}',
    }
    cell_style_base = {
        'fontSize': '13px', 'padding': '8px 10px', 'textAlign': 'right',
        'borderBottom': f'1px solid {theta_dashboard.C["border"]}',
    }
    cols = ['Symbol', 'Price', 'Chg%', 'P/C Ratio', 'Net GEX', 'IV Skew', 'Net Premium', 'Aggression']
    thead = html.Tr([html.Th(c, style={**header_style, 'textAlign': 'left' if c == 'Symbol' else 'right'}) for c in cols])
    rows = []
    for sym in ['SPXW', 'SPY', 'QQQ', 'IWM', 'TLT', 'VIXW']:
        if sym not in all_stats:
            continue
        st = all_stats[sym]
        chg = st.get('price_change', 0.0)
        chg_c = theta_dashboard.C['call'] if chg >= 0 else theta_dashboard.C['put']
        pc = st.get('pc_ratio', 0.0)
        pc_c = theta_dashboard.C['put'] if pc > 1.0 else theta_dashboard.C['call']
        gex = st.get('net_gamma', 0.0)
        gex_c = theta_dashboard.C['call'] if gex >= 0 else theta_dashboard.C['put']
        agg = st.get('trade_aggression', 0.0)
        agg_c = theta_dashboard.C['call'] if agg > 0 else theta_dashboard.C['put']
        rows.append(html.Tr([
            html.Td(sym, style={**cell_style_base, 'textAlign': 'left', 'fontWeight': 700, 'color': theta_dashboard.C['text']}),
            html.Td(f"${st['price']:.2f}", style={**cell_style_base, 'color': theta_dashboard.C['text']}),
            html.Td(f"{chg:+.2f}%", style={**cell_style_base, 'color': chg_c, 'fontWeight': 600}),
            html.Td(f"{pc:.3f}", style={**cell_style_base, 'color': pc_c, 'fontWeight': 600}),
            html.Td(f"{gex/1e3:.1f}K" if abs(gex) < 1e6 else f"{gex/1e6:.1f}M", style={**cell_style_base, 'color': gex_c}),
            html.Td(f"{st.get('iv_skew', 0.0):.4f}", style={**cell_style_base, 'color': theta_dashboard.C['text_sec']}),
            html.Td(_fmt_premium(st.get('net_premium', 0.0)), style={**cell_style_base, 'color': theta_dashboard.C['call'] if st.get('net_premium', 0.0) >= 0 else theta_dashboard.C['put']}),
            html.Td(f"{agg:+.3f}", style={**cell_style_base, 'color': agg_c, 'fontWeight': 600}),
        ]))
    return html.Div(
        style={
            'backgroundColor': theta_dashboard.C['bg_card'],
            'border': f'1px solid {theta_dashboard.C["border"]}',
            'borderRadius': '8px', 'padding': '16px', 'marginBottom': '20px', 'overflowX': 'auto',
        },
        children=[
...
app = dash.Dash(__name__, title="Theta Options Dashboard — Enhanced")
...
def update_dashboard(n, trigger, symbol, dte, compare, window, manual_refresh, paused):
    ...
    ticker_ribbon = _build_ticker_ribbon(latest_stats)
    ...
    if symbol != 'ALL' and symbol in latest_stats:
        ...
        pc = st.get('pc_ratio', 0.0)
        pc_color = theta_dashboard.C['put'] if pc > 1.0 else theta_dashboard.C['call']
        agg_val = st.get('trade_aggression', 0.0)
        agg_color = theta_dashboard.C['call'] if agg_val > 0 else theta_dashboard.C['put']
        agg_label = 'Buyers' if agg_val > 0 else 'Sellers'

        divider = html.Div(style={'width': '1px', 'backgroundColor': theta_dashboard.C['border'], 'margin': '0 4px', 'alignSelf': 'stretch'})

        stats_boxes = html.Div(style={'display': 'flex', 'gap': '6px', 'flexWrap': 'wrap', 'alignItems': 'center'}, children=[
            _stat_tile('Price', f"${st['price']:.2f}", theta_dashboard.C['text'], f"{st['price_change']:+.2f}%"),
            divider,
            _stat_tile('P/C Ratio', f"{pc:.3f}", pc_color, 'Bearish' if pc > 1.0 else 'Bullish'),
            divider,
            _stat_tile('Net GEX', f"{st['net_gamma']/1e6:.1f}M" if abs(st['net_gamma']) >= 1e6 else f"{st['net_gamma']/1e3:.1f}K",
                       theta_dashboard.C['call'] if st['net_gamma'] >= 0 else theta_dashboard.C['put']),
            divider,
            _stat_tile('IV Skew', f"{st.get('iv_skew', 0.0):.4f}", theta_dashboard.C['text_sec']),
            divider,
            _stat_tile('Net Premium', _fmt_premium(st.get('net_premium', 0.0)),
                       theta_dashboard.C['call'] if st.get('net_premium', 0.0) >= 0 else theta_dashboard.C['put']),
            divider,
            _stat_tile('ATM Straddle', f"{st.get('atm_straddle', 0.0):.2f}", theta_dashboard.C['accent']),
            divider,
            _stat_tile('Call IV / Put IV', f"{st.get('call_iv', 0.0):.2%} / {st.get('put_iv', 0.0):.2%}", theta_dashboard.C['text_sec']),
            divider,
            _stat_tile('Aggression', f"{agg_val:+.3f}", agg_color, agg_label),
        ])
    ...
    # --- Premium Flow Bar ---
    if symbol != 'ALL' and symbol in latest_stats:
        pf_bar = _build_premium_flow_bar(latest_stats[symbol])
        if pf_bar is not None:
            content.append(pf_bar)
    ...
    if symbol != 'ALL':
        ...
        if model_out is None:
            ...
            # 1.5 Market Regime Badge
            if symbol in latest_stats:
                vix_lvl = model_out.get('vix_level', 0.0) if model_out else None
                regime_badge = _build_regime_badge(latest_stats[symbol].get('net_gamma', 0.0), vix_lvl)
                content.append(regime_badge)
            ...
            if health_panel is not None:
                content.append(html.H3("Model Health", style={'color': theta_dashboard.C['text_sec'], 'marginTop': '30px', 'borderBottom': f'1px solid {theta_dashboard.C["border"]}', 'paddingBottom': '10px'}))
                content.append(health_panel)

            # 7. Alert Panel
            alert_panel = _create_alert_panel()
            content.append(html.H3("Alerts & Notifications", style={'color': theta_dashboard.C['text_sec'], 'marginTop': '30px', 'borderBottom': f'1px solid {theta_dashboard.C["border"]}', 'paddingBottom': '10px'}))
        ...
        cs_table = _build_cross_symbol_table(latest_stats)
        if cs_table is not None:
            content.append(cs_table)
