...
Usage:
    python scripts/stage2/train_stage2_agent_2d.py --target SPXW --horizon 30
"""
...
def _load_2d_model(symbol: str, horizon: int, device: torch.device):
    ...
    return model, int(ckpt.get('feat_dim', 325)), invert_signal
...
def _load_chain_split(symbol: str, horizon: int, split: str):
    ...
    ts_path = d / f'{split}_timestamps.npy'
    if ts_path.exists():
        timestamps = np.load(ts_path).astype(np.int64)
    else:
        timestamps = np.arange(len(labels), dtype=np.int64)
    return seq, chain, labels, nm, ns, timestamps


@torch.no_grad()
def _infer_frozen_2d(model, seq, chain, norm_mean, norm_std, device, batch_size=1024, invert_signal=False):
...
def _build_peer_features(horizon: int, split: str, frozen_models: dict,
                          spxw_ts: np.ndarray, device: torch.device, batch_size: int,
                          peer_symbols: list):
    """Pre-compute frozen peer 2D logits aligned by SPXW timestamps."""
    peer_logits = {}
    peer_probs = {}
    peer_ts = {}

    for sym in peer_symbols:
        seq, chain, _, nm, ns, ts = _load_chain_split(sym, horizon, split)
        model_obj, invert_signal = frozen_models[sym]
        lg, pb = _infer_frozen_2d(
            model_obj, seq, chain, nm, ns, device, batch_size, invert_signal=invert_signal
        )
        peer_logits[sym] = lg
        peer_probs[sym] = pb
        peer_ts[sym] = ts

    keep = np.ones(len(spxw_ts), dtype=bool)
    ts_maps = {}
    for sym in peer_symbols:
        ts_maps[sym] = {int(t): i for i, t in enumerate(peer_ts[sym])}
        keep &= np.isin(spxw_ts, peer_ts[sym])
    base_idx = np.flatnonzero(keep)
    aligned_ts = spxw_ts[base_idx]

    parts = []
    for sym in peer_symbols:
        idx = np.array([ts_maps[sym][int(t)] for t in aligned_ts], dtype=np.int64)
        parts.append(peer_logits[sym][idx].reshape(-1, 1))
        parts.append(peer_probs[sym][idx].reshape(-1, 1))

    peer_feat = np.concatenate(parts, axis=1).astype(np.float32)
    return peer_feat, base_idx, aligned_ts


@torch.no_grad()
def _build_tlt_context_2d(horizon: int, split: str, tlt_model,
                           device: torch.device, batch_size: int, invert_signal: bool = False) -> np.ndarray:
    """Run frozen TLT Stage1 2D model → (n, 2) array of [logit, prob]."""
    seq, chain, _, nm, ns, ts = _load_chain_split('TLT', horizon, split)
    lg, pb = _infer_frozen_2d(tlt_model, seq, chain, nm, ns, device, batch_size, invert_signal=invert_signal)
    return np.stack([lg, pb], axis=1).astype(np.float32), ts
...
def main():
    ...
    spxw_2d_model, spxw_feat_dim, spxw_invert = _load_2d_model(args.target, args.horizon, device)
    spxw_2d_model.train()
    logger.info(f"  Loaded SPXW Stage1 2D (unfrozen), feat_dim={spxw_feat_dim}, invert={spxw_invert}")

    frozen_peer_models = {}
    for sym in peer_symbols:
        try:
            m, _, invert_signal = _load_2d_model(sym, args.horizon, device)
            m.eval()
            for p in m.parameters():
                p.requires_grad_(False)
            frozen_peer_models[sym] = (m, invert_signal)
            logger.info(f"  Loaded {sym} Stage1 2D (frozen, invert={invert_signal})")
    ...
    if args.variant == 'tlt_gated':
        try:
            tlt_model, _, tlt_invert = _load_2d_model('TLT', args.horizon, device)
            tlt_model.eval()
            for p in tlt_model.parameters():
                p.requires_grad_(False)
            logger.info(f"  Loaded TLT Stage1 2D (frozen, gate context only, invert={tlt_invert})")
        except FileNotFoundError as e:
    ...
    spxw_seq_train, spxw_chain_train, train_labels, nm_spxw, ns_spxw, spxw_ts_train = _load_chain_split(args.target, args.horizon, 'train')
    spxw_seq_val,   spxw_chain_val,   val_labels,   _,       _,       spxw_ts_val   = _load_chain_split(args.target, args.horizon, 'val')
    spxw_seq_test,  spxw_chain_test,  test_labels,  _,       _,       spxw_ts_test  = _load_chain_split(args.target, args.horizon, 'test')

    logger.info("  Pre-computing frozen peer features...")
    peer_feat_train, spxw_idx_train, aligned_ts_train = _build_peer_features(
        args.horizon, 'train', frozen_peer_models, spxw_ts_train, device, args.stage1_batch, active_peers)
    peer_feat_val,  spxw_idx_val, aligned_ts_val = _build_peer_features(
        args.horizon, 'val',   frozen_peer_models, spxw_ts_val,   device, args.stage1_batch, active_peers)
    peer_feat_test, spxw_idx_test, aligned_ts_test = _build_peer_features(
        args.horizon, 'test',  frozen_peer_models, spxw_ts_test,  device, args.stage1_batch, active_peers)
    logger.info(
        f"  Timestamp alignment kept: train={len(spxw_idx_train):,}/{len(spxw_ts_train):,}, "
        f"val={len(spxw_idx_val):,}/{len(spxw_ts_val):,}, test={len(spxw_idx_test):,}/{len(spxw_ts_test):,}"
    )

    tlt_ctx_train = tlt_ctx_val = tlt_ctx_test = None
    if args.variant == 'tlt_gated':
        logger.info("  Pre-computing TLT gate context...")
        tlt_ctx_train, tlt_ts_train = _build_tlt_context_2d(args.horizon, 'train', tlt_model, device, args.stage1_batch, invert_signal=tlt_invert)
        tlt_ctx_val,   tlt_ts_val   = _build_tlt_context_2d(args.horizon, 'val',   tlt_model, device, args.stage1_batch, invert_signal=tlt_invert)
        tlt_ctx_test,  tlt_ts_test  = _build_tlt_context_2d(args.horizon, 'test',  tlt_model, device, args.stage1_batch, invert_signal=tlt_invert)
        for split_name, aligned_ts, tlt_ctx, tlt_ts in [
            ('train', aligned_ts_train, tlt_ctx_train, tlt_ts_train),
            ('val', aligned_ts_val, tlt_ctx_val, tlt_ts_val),
            ('test', aligned_ts_test, tlt_ctx_test, tlt_ts_test),
        ]:
            tlt_map = {int(t): i for i, t in enumerate(tlt_ts)}
            keep = np.array([int(t) in tlt_map for t in aligned_ts], dtype=bool)
            if split_name == 'train':
                spxw_idx_train = spxw_idx_train[keep]
                peer_feat_train = peer_feat_train[keep]
                aligned_ts_train = aligned_ts_train[keep]
                tlt_ctx_train = tlt_ctx[np.array([tlt_map[int(t)] for t in aligned_ts_train], dtype=np.int64)]
            elif split_name == 'val':
                spxw_idx_val = spxw_idx_val[keep]
                peer_feat_val = peer_feat_val[keep]
                aligned_ts_val = aligned_ts_val[keep]
                tlt_ctx_val = tlt_ctx[np.array([tlt_map[int(t)] for t in aligned_ts_val], dtype=np.int64)]
            else:
                spxw_idx_test = spxw_idx_test[keep]
                peer_feat_test = peer_feat_test[keep]
                aligned_ts_test = aligned_ts_test[keep]
                tlt_ctx_test = tlt_ctx[np.array([tlt_map[int(t)] for t in aligned_ts_test], dtype=np.int64)]

    spxw_seq_train = spxw_seq_train[spxw_idx_train]
    spxw_chain_train = spxw_chain_train[spxw_idx_train]
    train_labels = train_labels[spxw_idx_train].astype(np.float32)
    spxw_seq_val = spxw_seq_val[spxw_idx_val]
    spxw_chain_val = spxw_chain_val[spxw_idx_val]
    val_labels = val_labels[spxw_idx_val].astype(np.float32)
    spxw_seq_test = spxw_seq_test[spxw_idx_test]
    spxw_chain_test = spxw_chain_test[spxw_idx_test]
    test_labels = test_labels[spxw_idx_test].astype(np.float32)
    ...
    train_ds = Chain2DJointDataset(
        spxw_seq_train, spxw_chain_train,
        peer_feat_train, train_labels, nm_spxw, ns_spxw,
        tlt_ctx=tlt_ctx_train if tlt_ctx_train is not None else None)
