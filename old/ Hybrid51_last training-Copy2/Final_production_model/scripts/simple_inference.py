...
def main():
    ...
    # Load VIX features
    vix_path = Path(args.vix_root) / f"{split}_vix_features.npy"
    vix_data = np.load(vix_path)
    if vix_data.ndim == 3:
        vix_data = vix_data[:, -1, :]
    vix_feat = _resample_to_length(vix_data.astype(np.float32), n_final)
    vix_feat = np.nan_to_num(vix_feat, nan=0.0, posinf=0.0, neginf=0.0)

    # Load Stage 3 model
    ckpt3 = torch.load(model_dir / "stage3/stage3_vix_gated.pt", map_location='cpu', weights_only=False)
    ...
    with torch.no_grad():
        logits3, gates3, _ = model3(
            torch.from_numpy(agent_mat).to(device),
            torch.from_numpy(vix_feat).to(device),
        )
        final_probs = torch.sigmoid(logits3).cpu().numpy().astype(np.float32)
        gates_np = gates3.cpu().numpy().astype(np.float32)

    threshold = float(ckpt3.get('threshold', 0.5))
